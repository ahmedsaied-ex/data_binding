...
class SharedPrefs private constructor(context: Context) {
    ...
    // Overloaded reading methods
    fun read(key: String, defaultValue: Set<String>): Set<String> =
    ...
    fun getSize(): Long =
        prefs.all.toString().toByteArray().size.toLong()          //Return Preferences size

    fun remove(key: String): String {
        var removedVal = ""
        if (getSize() > 0) {
            removedVal = prefs.getString(key, "Not Found").toString(); prefs.edit().remove(key)
                .apply()
        }
        return removedVal
    }
    ...
    companion object {
        private const val SHARED_PREFS_NAME = "my_preferences"
